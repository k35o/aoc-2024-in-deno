...
nkTransitionMap.set('8-A', [['right', 'bottom', 'bottom', 'bottom'], ['bottom', 'bottom', 'bottom', 'right']]);
...
nkTransitionMap.set('4-9', [['top', 'right', 'right'], ['right', 'right', 'top']]);
...
for (const line of lines) {
  const re = (operations: DKKey[], count: number) => {
    ...
    for (let i = 1; i < aOperations.length; i++) {
      len += Math.min(
        ...dkTransitionMap.get(`${aOperations[i - 1]}-${aOperations[i]}`)!.map((routes) => re(routes, count - 1))
      )
    }
